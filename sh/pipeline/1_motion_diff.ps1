...
$EXTRA = @(
    "--tcp"         # RTSP через TCP — ВКЛ: UDP по Wi-Fi терял пакеты → столлы потока (~43% активного
                    #   времени, дыры кратно stimeout=3с) → мало кадров в сцене. TCP ретрансмитит потери.
    # "--cam-ts"    # добавлять время камеры из RTCP NTP в имя файла (планируется для фикса времени кадров)
)
