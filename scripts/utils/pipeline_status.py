...
def fmt_stats(st: dict, has_timing: bool, cpu_line: str | None = None,
    ...
    lines = [head]
    # Тайминг-строка — единый формат с status_win.ps1: '<ед>/кадр мин/ср/макс' (единица впереди, 3 значения,
    # БЕЗ 'посл'). Ярлык-операция не пишется — тип виден по ряду сервиса. Всё — ЧИСТОЕ время операции:
    # compute → мсек/кадр, I/O файла (приём/отправка) → сек/кадр.
    if kind == "burst":
        # transfer: чистое время приёма файла (I/O) — успевает ли приём
        if has_timing and st.get("avg") is not None:
            lines.append(f"сек/кадр {st['min']}/{st['avg']}/{st['max']}")
    else:
        # compute-сервисы: ЧИСТОЕ время вычисления кадра (только счёт, без сна) → успевает ли ЦПУ
        if st.get("cavg") is not None:
            lines.append(f"мсек/кадр {st['cmin']}/{st['cavg']}/{st['cmax']}")
...
def render_md(rows: list[dict], ts: str) -> str:
    ...
    headers = ["Сервер", "Сервис", "Статус", "Вход<br>(N файл. + послед.)", "Выход<br>(N файл. + послед.)",
               "Лог: работа", "Лог: ожид.",
               "прогонов (кадров/файлов) (за 10м/1ч/24ч)<br>&lt;мсек&#124;сек&gt;/кадр (мин/ср/макс)<br>цпу% (мин/ср/макс/посл)"]
    data = [
        ["Linux",
         f"`{r['label']}`",
         ("✓ " if r["state"] == "active" else "✗ ") + r["state"],
