...
def fmt_stats(st: dict, has_timing: bool, cpu_line: str | None = None,
    ...
    lines = [head]
    # Тайминг-строка — единый формат с status_win.ps1: 'ярлык мин/ср/макс <ед>/кадр' (3 значения, БЕЗ 'посл').
    # Все ярлыки — ЧИСТОЕ время операции: счёт (compute, мсек/кадр), приём/отправка (I/O файла, сек/кадр).
    if kind == "burst":
        # transfer: чистое время приёма файла (I/O) — успевает ли приём
        if has_timing and st.get("avg") is not None:
            lines.append(f"приём {st['min']}/{st['avg']}/{st['max']} сек/кадр")
    else:
        # compute-сервисы: ЧИСТОЕ время вычисления кадра (только счёт, без сна) → успевает ли ЦПУ
        if st.get("cavg") is not None:
            lines.append(f"счёт {st['cmin']}/{st['cavg']}/{st['cmax']} мсек/кадр")
...
def render_md(rows: list[dict], ts: str) -> str:
    ...
    headers = ["Сервер", "Сервис", "Статус", "Вход<br>(N файл. + послед.)", "Выход<br>(N файл. + послед.)",
               "Лог: работа", "Лог: ожид.",
               "прогонов (кадров/файлов) (за 10м/1ч/24ч)<br>счёт·приём·отправка (мин/ср/макс)<br>цпу% (мин/ср/макс/посл)"]
    data = [
        ["Linux",
         f"`{r['label']}`",
         ("✓ " if r["state"] == "active" else "✗ ") + r["state"],
