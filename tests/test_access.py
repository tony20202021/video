...
def test_parse_single_ip():
    rules = parse_allowed_ips("109.252.161.115")
    assert rules is not None
    assert rules.allows("109.252.161.115")
    assert rules.allows("127.0.0.1")
    assert not rules.allows("10.0.0.1")


def test_parse_cidr_subnet():
    rules = parse_allowed_ips("109.252.161.0/24")
    assert rules.allows("109.252.161.115")
    assert rules.allows("109.252.161.1")
    assert not rules.allows("109.252.162.1")
...
def test_log_ip_denied_with_valid_key(caplog):
    ...
    log_ip_denied(
        ...
        client_ip="109.252.161.200",
        service="Transfer",
        path="/file",
        api_key_valid=True,
    )
    assert "API key OK" in caplog.text
    assert "109.252.161.0/24" in caplog.text
