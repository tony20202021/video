...
**Время = момент СЪЁМКИ кадра (PTS), с ДРЕЙФ-ГАРДОМ.** `1_motion_diff.py` штампует имя по PTS кадра
(`CAP_PROP_POS_MSEC`), привязанному к wall-clock ПК якорем `(pts0, pc0)` на URL (сброс при реконнекте):
`время = pc0 + (pts − pts0)`. НО у нестабильного субпотока (по TCP; реконнекты «40 битых кадров»; PTS
рваный/замерзает) якорь ПРОТУХАЕТ и метка уезжает от реального времени на ЧАСЫ, хотя поток ЖИВОЙ
(проверено по OSD камеры: кадр с впечатанным OSD-временем 18:11 имел имя 23:42 предыдущего дня, −17ч; это
был баг МЕТКИ, а не задержка доставки). Поэтому: **дрейф-гард `resolve_cap_ts` (`motion_utils.py`)** — если
`|wall − pts_время| > MOTION_PTS_DRIFT_MAX_SEC` (деф. 60с), якорь считается протухшим → переякорь на wall и
штамп по wall (кадры живые → wall ≈ истинная съёмка в пределах секунд; переякорь чище реконнекта — не рвёт поток).
Фолбэк на wall и при недоступном PTS (`pts ≤ 0`). Сырой PTS остаётся в `pts.csv` для диагностики; срабатывания
гарда видны в статус-отчёте (строка `[!] PTS-дрейф Nx → штамп по wall` у motion_diff). Строгая альтернатива —
камерное NTP-время через RTCP SR (`--cam-ts`, `_RtcpWorker`; в проде выключено).
