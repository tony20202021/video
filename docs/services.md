...
`время обраб.` (мин/ср/макс/посл) с подписью единицы (`1прогон`/`1кадр`), `цпу%`
(мин/ср/макс/посл). CPU — из `cpu.csv` сервиса с тем же фолбэком окон.
...
- **Батч-стадии** `video-yolo/classify/identify` — `N прогонов (X кадров)`: `N` — poll-циклы
  ...
  `10 прогонов (0 кадров)` = 10 прогонов, детекций нет). Тайминг подписан `1прогон` — время на батч.
- **Непрерывный приём** `video-transfer` — `N прогонов (X файлов)`: `N` — всплески приёма
  (события, разделённые паузой > `BURST_GAP_SEC`), `X` — принятые файлы. Тайминг `1кадр`.
- **Windows** (`status_win.ps1`): `motion_diff` — `1 прогон (X файлов)` (непрерывный цикл, X = сохранённые
  кадры с движением); `2_send` — `N прогонов (X файлов)` (N = батчи отправки `[в батче 1/K]`).
