...
2. **`счёт min/avg/max мсек/кадр`** — ЧИСТОЕ время вычисления кадра (только счёт, без сна
   ...
   `camera_run.compute_per_frame_log`; в логе единица `мс`, в отчёте показывается как `мсек/кадр`).
   У `video-transfer` вместо этого `приём min/avg/max сек/кадр` (I/O приёма файла), у Windows `2_send` —
   `отправка min/avg/max сек/кадр` (I/O отправки). Все три — ЧИСТОЕ время операции, единый формат
   `ярлык мин/ср/макс <мсек|сек>/кадр` (3 значения); idle-интервалы (с простоем) **не показываются**;
