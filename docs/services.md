...
- **Индикатор PTS-дрейфа** (только `motion_diff`): если за окно 10м/1ч в `run.log` были WARNING-строки
  дрейф-гарда, в stats-колонке добавляется `[!] PTS-дрейф Nx → штамп по wall` — сигнал, что субпоток
  нестабилен и метки времени чинятся по wall (см. дрейф-гард в [cameras.md](cameras.md)). Нет строки = нет дрейфа.
...
**Графики cam3** (`_cam3_cpu_fps.png`, `_cam3_pts_drift.png`) сервер строит из CSV камеры
(`1_motion_diff.py --regen-from`). На diff-панелях порог рисуется **по зонам**: если у зон разные
пороги (`<VAR>_MOTION_THRESHOLD`) — линия на каждую зону в её цвете (пороги едут в `run_params.json`,
т.к. regen на сервере не знает камерный `.env`); при равных — одна общая линия.

**Полный день (склейка на сервере).** CSV лежат в каталоге WALL-даты (`images/<день>/`), а рестарт
прогона ПЕРЕЗАПИСЫВАЕТ дневной CSV (`'w'`) → штатный график может показать день не целиком. Инструмент
`scripts/utils/regen_cam_day.py <YYYYMMDD>` собирает ПОЛНЫЙ день на СЕРВЕРЕ: тянет по scp CSV за дату и
соседние (D-1,D,D+1 — строки могут быть размазаны), фильтрует по `ts_msk==дата`, переписывает `mono_s`
в секунды суток (иначе ось ломается на стыке прогонов с обнулённым mono), сшивает и рендерит. Камеру
только читаем (scp), обработка — на сервере. Для дней ДО фикса меток PTS результат частичный (кадры
мис-датированы под другие даты — не восстановить); для дней ПОСЛЕ фикса — полный день.
`--out-cpu`/`--out-pts` — куда положить PNG.
