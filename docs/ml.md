...
|----------|-----|----------|-------|
| **Жёсткий голос** | `SMOOTH_HARD_VOTE` | **1** (вкл) | **АКТИВНЫЙ метод:** 1 класс на весь визит = argmax среднего probs (вместо Viterbi). Замер v3/v4: 11.6%/16.3% — лучший. Требует `MERGE_ZONES=1` (иначе топит ЖКХ). 0 → вернуть Viterbi |
| Склейка зон | `SMOOTH_MERGE_ZONES` | **1** (вкл) | склеить зоны `d`/`u` ОДНОЙ камеры в один поток по времени (разные физ. камеры — раздельно). Обязательна с `HARD_VOTE`: чинит recall ЖКХ (35→55) |
| Режим окна | `SMOOTH_PROB_WINDOW` | **0** | резервный: 0 = Viterbi/голос; >0 = бегущее окно ±N/2 сек. Игнорируется при `HARD_VOTE=1` |
| Пауза визита | `SMOOTH_GAP_SEC` | **10** | пауза > 10с → новый проход (замер gap 5..60 → плато с 10) |
| Липкость (Viterbi) | `SMOOTH_P_STAY` | **0.9** | вероятность сохранить класс между кадрами. Действует ТОЛЬКО при `HARD_VOTE=0` (резервный Viterbi) |
| Гейт | `SMOOTH_GATE` | **0** (выкл) | не трогать уверенные (max prob ≥ N). Действует только в Viterbi/окне; голос игнорирует |

Приоритет режимов в `smooth_sequence`: **`HARD_VOTE` → окно (`PROB_WINDOW>0`) → Viterbi**. Предохранители
...
физ. камеры `cam_01`/`cam_02` остаются раздельными). **С Viterbi эффект был в пределах шума** (v3 −0.17 п.п.),
но **с жёстким голосом склейка обязательна** — без неё голос топит ЖКХ (recall 48→35), со склейкой чинит
(→55). Поэтому в проде `MERGE_ZONES=1` + `HARD_VOTE=1` вместе.

**Свип методов/гиперпараметров vs РУЧНОЙ истины (v3 — 9 дат 9481 кропов; v4 — 12 дат 14941).**
Ошибка = сглаж-класс ∉ истины; recall по КЛАССУ = на single-label истине v4 (res=житель, del=доставка,
utl=ЖКХ, gst=гость):

| Метод | v3 | v4 | recall res/del/utl/gst (v4) |
|-------|----|----|--------|
| baseline (argmax/кроп) | 23.6% | 31.0% | 72/61/48/56 |
| Viterbi p_stay=0.7 | 20.2% | 24.5% | 80/62/43/60 |
| current (Viterbi p_stay=0.9) | 18.8% | 22.9% | 82/62/45/59 |
| Viterbi p_stay=0.95 | 18.2% | 22.1% | 83/61/45/59 |
| Viterbi p_stay=0.99 | 16.8% | 20.8% | 84/60/45/61 |
| окно ±3с (k7,tri) | 21.0% | 27.0% | 77/61/46/60 |
| окно ±5с | 14.9% | 20.3% | 85/60/**35**/64 |
| жёсткий голос: 1 класс/визит | 12.9% | 18.2% | 87/62/**35**/64 |
| Viterbi p_stay=0.99 + merge-зон | 16.4% | 20.3% | 85/61/53/61 |
| **жёсткий голос + merge-зон** | **11.6%** | **16.3%** | **89/62/55/68** |
| oracle (лучший 1 класс/визит по истине) | 3.6% | 2.6% | — |

Выводы: (1) **oracle ~3% → визиты почти однокласные**, значит группировка «камера-визит» УЖЕ хорошая, а
**трекинг по персонам — НЕ рычаг** (боксы кропов теперь сохраняются в имени `_b<x1>-<y1>-<x2>-<y2>` —
можно измерить позже, но потолок задаёт oracle ~3%). (2) **Рычаг — метод**: жёсткое «один класс на визит»
(argmax среднего probs) вместо Viterbi даёт **−6/−5 п.п.** — на порядок больше склейки зон. (3) Остаток
до oracle — точность МОДЕЛИ (ошибки на целых визитах), не сглаживание.
✅ **Опасение о меньшинствах разрешено замером:** жёсткий голос БЕЗ склейки топит ЖКХ (utl recall 48→**35**;
класс уходит в доминирующего резидента) — как и опасались. Но **склейка зон это лечит** (35→**55**, выше
baseline). `жёсткий голос + merge-зон` — единственный метод, который бьёт baseline и по ошибке, и по recall
ВСЕХ 4 классов сразу (res 89, del 62, utl 55, gst 68). Viterbi щадит меньшинства и без склейки (utl держится
43–45 при любом `p_stay`), но и общий выигрыш вдвое меньше. **ВКЛЮЧЕНО В ПРОД: `SMOOTH_HARD_VOTE=1` +
`SMOOTH_MERGE_ZONES=1`** (режим «жёсткий голос» реализован в `smooth_sequence`, приоритетнее Viterbi/окна;
встроенный режим воспроизвёл замер 1-в-1: v3 11.6% / v4 16.3%). Направление/`tracker.py` — прототип, не в пайплайне.
