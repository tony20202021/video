...
43–45 при любом `p_stay`), но и общий выигрыш вдвое меньше. **Кандидат в прод: жёсткий голос по визиту +
`SMOOTH_MERGE_ZONES=1`** (сейчас в `smooth_core` нет режима «жёсткий голос» — нужна реализация как выбираемого
smoother-режима; Viterbi/окно уже есть). Направление/`tracker.py` — прототип, не в пайплайне.
