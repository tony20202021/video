...
**Метод** (`src/common/utils/temporal_smooth.py`, скрипт `scripts/pipeline/3b_smooth_groups.py`):
1. по `classifications.csv` строим последовательности **(камера, время, вероятности p_<class>)**;
2. **сегментация на визиты** по паузам во времени (пауза > `gap` → новый визит);
3. **HMM / Viterbi ВНУТРИ визита**: переходы `p_stay` («человек не телепортируется между
   классами»), эмиссия = вероятности модели. Класс МОЖЕТ меняться внутри визита, поэтому короткая
   доставка в потоке резидентов НЕ затирается (в отличие от «1 класс на визит»);
4. опциональный **гейтинг**: уверенные предсказания (max prob ≥ `gate`) не трогаем;
5. если сглаженный класс ≠ текущей раскладки → кроп перекладывается в `single/<class>/`,
   правится `labels.json`, `uncertain`-кропы получают класс визита (спасаются); пишется
   `classifications_smoothed.csv` (аудит `model_class → smoothed_class`). `multi`/`unknown` не трогаются.

**Осторожно с многолюдными кадрами.** У двери часто НЕСКОЛЬКО людей разных классов одновременно
(кропы `pNofM`, M>1 — разные люди в одном кадре). Сглаживать их по «камера+время» НЕЛЬЗЯ — оно
схлопывает разных людей в один класс (замер: «сглаживать всё» роняет recall гостя 0.96→0.88).
Поэтому по умолчанию **сглаживаются только одиночные кадры (M=1)** + **гейтинг** (уверенные
предсказания не трогаем). Полноценно эту проблему решает трекинг по персонам (пока не сделан).

**Замеры:**
- слабая модель v3_1 (9 дат, 9419 кропов): baseline **25.7%** → visit-HMM ~18% (много чего чинить);
- точная модель **v4_1** (день 20260720, 1001 кроп): baseline **7.0%** → **M=1 + gate0.8 = 5.4%**,
  при этом ВСЕ меньшинства сохранены (доставка 1.00, гость 0.95, ЖКХ 1.00), резидент 0.92→0.94.
  «Сглаживать всё» даёт 5.3%, но ломает гостя (0.88). «1 класс на визит» — 11.6% ошибок у v3_1,
  но роняет доставку 0.53 — отвергнут.

Чем точнее модель, тем меньше выигрыш сглаживания (7.0→5.4% у v4_1 vs 25.7→18% у v3_1) — но при
`M=1 + gate` оно **безопасно** (не портит меньшинства). Дефолты: `gap=60с`, `p_stay=0.95`,
`gate=0.8`, `max_persons=1` (env `SMOOTH_GAP_SEC`/`SMOOTH_P_STAY`/`SMOOTH_GATE`/`SMOOTH_MAX_PERSONS`).
Скорость не важна (офлайн), считается от исходных вероятностей CSV → идемпотентно. Сглаживать
имеет смысл СВЕЖИЙ инференс (не размеченный вручную — там метки уже истина).

**Дебаг-визуализация** (`--viz`, env `SMOOTH_VIZ=1`, на начальном этапе включена): для КАЖДОГО
исправленного кропа рисуется конкат кадров ВИЗИТА (на основе которых Viterbi принял решение) в
`<date>/meta/smooth_viz/<crop>.jpg`. Исправленный кадр — в жёлтой рамке; под каждым кадром время
и класс модели (argmax); сверху вероятности модели по всем классам и новый класс. `meta/` не
попадает в раскладку кропов (identify его игнорирует). Выключить: `SMOOTH_VIZ=0` или `--no-viz`.

**Порядок пайплайна:** classify (3) → **smooth (3b)** → identify (4). Все три — независимые
watch-циклы в ОДНОМ каталоге `.data/groups/<ver>/inference/images/<date>/`: classify раскладывает
`single/<class>/`+`multi/`+CSV+labels.json; 3b in-place перекладывает `single/` и правит labels.json;
identify читает `single/{1_resident,4_guest}`+`multi/` и пишет свой выход в ОТДЕЛЬНОЕ дерево
`.data/residents/v1/inference/` (свои CSV+labels.json — с groups не конфликтует).

**Синхронизация identify ← smooth (watermark).** systemd `After=` задаёт только порядок старта,
не пофайловую готовность. Без координации identify успевает опознать «ложного резидента» (ошибку
модели) РАНЬШЕ, чем 3b переложит кроп → неверная личность не откатывается (identify идемпотентен).
Решение: 3b пишет `<date>/meta/smooth_state.json` = `{csv_rows: N}` (докуда просглажен
classifications.csv). identify (`4_...sh`) берёт дату только когда `csv_rows ≥` текущего числа строк
CSV. Env `SMOOTH_WAIT=1` (вкл), `SMOOTH_WAIT_TIMEOUT=600` — fallback: если CSV не менялся N сек
(smooth выключен/отстал), не блокируем (нет дедлока). Пропущенный резидент (обратная ошибка)
самолечится: 3b переложит в `single/1_resident` → identify возьмёт на следующем поллинге.

⚠️ 3b владеет `labels.json` живого инференса (модель+сглаживание) — ручную разметку под датасет
вести на СНАПШОТЕ (копии каталога), иначе 3b затрёт правки. Разметчик игнорирует `meta/`
(viz-конкаты и watermark не показываются как кропы).
