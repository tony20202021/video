...
**Замер** (v3_1, 9 дат, 9419 размеченных кропов): baseline **25.7%** ошибок → visit-HMM **~18%**;
recall доставки **0.68 → 0.69** (сохраняется), резидента 0.75 → 0.83. «1 класс на визит» даёт 11.6%
ошибок, но роняет доставку до 0.53 — поэтому берём именно HMM внутри визита. Дефолты: `gap=60с`,
`p_stay=0.95` (env `SMOOTH_GAP_SEC`/`SMOOTH_P_STAY`/`SMOOTH_GATE`). Скорость не важна (офлайн на
сервере), считается от исходных вероятностей CSV → идемпотентно.
