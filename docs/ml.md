...
**Замер группировки/метода vs РУЧНОЙ истины (v3 — 9 дат 9481 кропов; v4 — 12 дат 14941):**

| Метод | v3 | v4 |
|-------|----|----|
| baseline (argmax/кроп) | 23.6% | 31.0% |
| current (Viterbi p_stay=0.9) | 18.8% | 22.9% |
| **жёсткий голос: 1 класс/визит (по probs модели)** | **12.9%** | **18.2%** |
| жёсткий голос + merge-зон | 11.6% | 16.3% |
...
**трекинг по персонам — НЕ рычаг** (плюс ретроспективно его не измерить: боксы кропов не сохраняются —
`classifications.csv` без боксов, `detections.csv` перезаписывается). (2) **Рычаг — метод**: жёсткое
«один класс на визит» (argmax среднего probs) вместо Viterbi даёт **−6/−5 п.п.** — на порядок больше
склейки зон. (3) Остаток до oracle — точность МОДЕЛИ (ошибки на целых визитах), не сглаживание.
⚠️ Жёсткий голос может топить меньшинства (ЖКХ/доставка в смешанных визитах) — Viterbi выбирали ради их
сохранения; перед сменой прода проверить per-class recall. Направление/`tracker.py` — прототип, не в пайплайне.
