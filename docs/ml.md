...
| Задача | Модель | Размер | Скорость на CPU* |
|--------|--------|--------|-----------------|
| Детекция людей | YOLOv8n | ~6 MB | ~20–40 мс/кадр |
| Классификация группы | MobileNetV3-Small | ~10 MB | ~5–10 мс/crop |
| Детекция лица | RetinaFace-MobileNet | ~1.6 MB | ~10 мс |
| Идентификация | MobileFaceNet | ~4 MB | ~5 мс/лицо |

*Современный CPU (i5/i7), одно изображение, ONNX Runtime.

Суммарный размер всех моделей ≈ 22 MB — Git LFS не нужен, хранятся напрямую в репозитории.
...
Активная версия каждой модели указывается в `config.yaml`. После дообучения новая версия кладётся рядом, активируется через API.
