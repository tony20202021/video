...
|----------|-----|----------|-------|
| Режим | `SMOOTH_PROB_WINDOW` | **0** | 0 = Viterbi (активен); >0 = бегущее окно ±N/2 сек |
| Пауза визита | `SMOOTH_GAP_SEC` | **10** | пауза > 10с → новый проход (замер gap 5..60 → плато с 10) |
| Липкость | `SMOOTH_P_STAY` | **0.9** | вероятность сохранить класс между кадрами (выше → меньше флипов, но давит меньшинства) |
| Гейт | `SMOOTH_GATE` | **0** (выкл) | **КРИТИЧНО:** с гейтом 0.8 Viterbi слабее (21.1%) — уверенные-но-неверные не флипаются |
| Склейка зон | `SMOOTH_MERGE_ZONES` | **0** (выкл) | склеить зоны `d`/`u` ОДНОЙ камеры в один поток по времени (разные физ. камеры — раздельно). Замер: см. ниже |

Предохранители (`guard`/`min-minority`/`veto`) — выкл по умолчанию. Параметры окна
(`SMOOTH_PROB_WINDOW_K`, `_TRI`) не используются при `PROB_WINDOW=0` (оставлены для быстрого возврата к окну).
...
физ. камеры `cam_01`/`cam_02` остаются раздельными). **Замер vs ручная истина:** на 3 днях v4 (2637 кропов)
склейка дала −0.7 п.п. (12.7→12.0%), но на 9 днях v3 (9481) — лишь **−0.17 п.п.** (в пределах шума) →
**по умолчанию ВЫКЛ**. Существенный прирост даст только трекинг по персонам (не сделан).
